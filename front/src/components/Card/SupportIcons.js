            ...
            { support.map( (s) => 
                <Grid item xs={xs} sx={{ justifyContent: "center", alignItems:"center" }} key={key}>
                    <Item sx={{  display: "flex", flexDirection: "column"}}>
                        ...
                        { s==="Matériel" && <TableRestaurantOutlinedIcon sx={{ color: theme.palette.popRed.main, fontSize: 36 }}/> }
                        { s==="Intervenants" && <PeopleIcon sx={{ color: theme.palette.popGreen.main, fontSize: 36 }} /> }
